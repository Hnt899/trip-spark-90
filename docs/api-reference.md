...
Инициация платежа, возврат URL для редиректа в ЮKassa.

### POST `/api/yookassa-sync-payment`
Синхронизация статуса оплаты после возврата на страницу успеха.

### POST `/api/yookassa-webhook`
Webhook от ЮKassa (события `payment.succeeded` и `payment.canceled`).

### POST `/api/tickets/:id/refund`
Возврат на карту через ЮKassa (JWT, только свой оплаченный заказ). Body: `{ "amount": 1234.5 }` (опционально, по умолчанию полная сумма).

### POST `/api/admin/tickets/:id/refund`
Возврат платежа через ЮKassa (только admin).
