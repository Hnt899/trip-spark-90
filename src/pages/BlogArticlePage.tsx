...
import ArticleTocSidebar from "@/components/blog/ArticleTocSidebar";
import SimilarArticlesSidebar, {
  SimilarArticlesMobile,
} from "@/components/blog/SimilarArticlesSidebar";
    ...
    <div className="min-h-screen bg-white dark:bg-background">
      ...
      <main className="px-4 py-[calc(var(--site-header-height)+1.5rem)] pb-16 md:px-6">
        <div className="mx-auto w-full max-w-[1440px]">
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,720px)_minmax(220px,280px)] lg:gap-x-0 xl:grid-cols-[minmax(0,1fr)_minmax(0,720px)_minmax(240px,300px)]">
            {/* sticky на grid-item + self-start — едет вместе с прокруткой */}
            <div className="sticky top-[calc(var(--site-header-height)+1.25rem)] z-20 hidden self-start lg:block lg:justify-self-end lg:pr-10 lg:pt-28 xl:pr-16 xl:pt-32">
              <ArticleTocSidebar blocks={contentBlocks} />
            </div>

            {/* Центр — статья */}
            <article className="min-w-0 w-full max-w-[720px] justify-self-center break-words lg:px-6 xl:px-8">
              ...
              {article ? (
                <div className="mt-10 min-w-0">
                  ...
                  <SimilarArticlesMobile articles={relatedPosts} className="mt-10" />
                </div>
              ) : (
            ...
            {/* Правая колонка — похожие статьи */}
            <div className="sticky top-[calc(var(--site-header-height)+1.25rem)] z-20 hidden self-start lg:block lg:pl-4 lg:pt-28 xl:pt-32">
              <SimilarArticlesSidebar articles={relatedPosts} />
            </div>
          </div>
        </div>
      </main>
