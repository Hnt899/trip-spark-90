...
// адрес API: локально — localhost:4000, в проде на Vercel — /api/support/chat
const API_URL =
...
const ChatWidget = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    if (typeof window !== "undefined") {
      const saved = window.localStorage.getItem("tudasuda-chat-history");
      if (saved) {
        try {
  ...
  const handleClose = () => {
    setIsChatOpen(false); // историю НЕ трогаем
  };

  return (
    <div className="pointer-events-none">
      {/* Кнопка открытия чата — всегда поверх */}
      {!isChatOpen && (
        <button
          type="button"
